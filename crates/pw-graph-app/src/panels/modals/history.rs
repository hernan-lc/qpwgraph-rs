...
fn history_block(ui: &mut Ui, document: &UiDocument, title: &str, empty: &str, entries: &[String]) {
    ui.label(
        RichText::new(title)
            .strong()
    ...
    if entries.is_empty() {
        ui.label(RichText::new(empty).color(document.theme_color(ThemeToken::TextWeak)));
    } else {
