...
// Some recent bindgen versions treat spa_pod_builder as opaque because it
// contains callback ABI details. Its public C layout is still stable and is
// required when the backing Vec grows after an overflow callback.
#[repr(C)]
struct SpaPodBuilderLayout {
    data: *mut c_void,
    size: u32,
    _padding: u32,
...
impl<'d> Builder<'d> {
    unsafe extern "C" fn overflow(data: *mut c_void, size: u32) -> c_int {
        ...
        let builder = &mut *(&mut (*this).builder as *mut spa_sys::spa_pod_builder
            as *mut SpaPodBuilderLayout);
        builder.data = (*this).data.as_mut_ptr().cast::<c_void>();
        builder.size = (*this)
            .data
            .len()
            .try_into()
