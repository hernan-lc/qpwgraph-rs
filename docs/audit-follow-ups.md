...
| UDP wildcard → interface migration | Migrating a host's audio socket from `0.0.0.0:PORT` to a specific address on the same port now removes the socket from its slot, waits for outstanding worker leases to drain within a bounded window, and closes it *before* the new bind. Previously only the `Arc` was unlinked, so a lease kept the wildcard socket open and the specific bind failed with `EADDRINUSE`. The reverse direction is handled identically. A failed bind reinstalls a wildcard socket on the **negotiated** port, and a drain that times out restores the still-live socket; both are reported as recoverable and the session carries on. Losing the negotiated port itself is fatal: a host never falls back to an ephemeral port, because the peer keeps sending to the port it negotiated and a resume does not renegotiate one. A fatal migration makes the host answer `PairFail` and tear the session down instead of `ResumeOk`, so the peer negotiates a fresh session rather than holding a live control link over black-holed audio. Same-interface resume still installs nothing. |
